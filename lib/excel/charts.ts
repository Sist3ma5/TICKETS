...
export interface ChartSpec {
  ...
  /**
   * Color de cada punto, en hex. En el pastel pinta las rebanadas y en las
   * barras cada barra, para que respeten los colores de la aplicación.
   */
  colors?: string[]
  /** Color único de la serie, cuando los datos no tienen color propio. */
  seriesColor?: string
  /** Tamaño en celdas. Por defecto, una gráfica amplia y legible. */
  size?: { cols: number; rows: number }
}

/** Ancho y alto por defecto, en celdas. */
const DEFAULT_SIZE = { cols: 13, rows: 28 }
...
/** Normaliza '#a78bfa' a 'A78BFA', que es lo que espera OOXML. */
function hex(color: string): string {
  return color.replace('#', '').toUpperCase()
}

/** Un punto coloreado — así cada rebanada o barra usa el color de la app. */
    ...
    .map(
      (color, i) =>
        `<c:dPt><c:idx val="${i}"/><c:bubble3D val="0"/><c:spPr><a:solidFill><a:srgbClr val="${hex(
          color,
        )}"/></a:solidFill><a:ln w="12700"><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill></a:ln></c:spPr></c:dPt>`,
    )
    .join('')
}

function seriesXml(spec: ChartSpec): string {
  ...
  // El orden de los hijos de <c:ser> lo fija el esquema: idx, order, tx,
  // spPr, dPt, dLbls, cat, val. Alterarlo hace que Excel rechace el archivo.
  const spPr = spec.seriesColor
    ? `<c:spPr><a:solidFill><a:srgbClr val="${hex(spec.seriesColor)}"/></a:solidFill></c:spPr>`
    : ''

  return `<c:ser><c:idx val="0"/><c:order val="0"/>
    <c:tx><c:strRef><c:f>${name}</c:f></c:strRef></c:tx>
    ${spPr}
    ${spec.colors?.length ? dPtXml(spec.colors) : ''}
...
function pieChartXml(spec: ChartSpec): string {
  return `<c:pieChart><c:varyColors val="1"/>${seriesXml(spec)}
    ${DLBLS}
...
function barChartXml(spec: ChartSpec): string {
  ...
  return `<c:barChart><c:barDir val="bar"/><c:grouping val="clustered"/><c:varyColors val="0"/>
    ${seriesXml(spec)}
    ${DLBLS}
    <c:gapWidth val="45"/><c:axId val="111111111"/><c:axId val="222222222"/></c:barChart>
    <c:catAx><c:axId val="111111111"/><c:scaling><c:orientation val="maxMin"/></c:scaling><c:delete val="0"/><c:axPos val="l"/><c:txPr><a:bodyPr/><a:lstStyle/><a:p><a:pPr><a:defRPr sz="1000"/></a:pPr><a:endParaRPr lang="es-MX"/></a:p></c:txPr><c:crossAx val="222222222"/></c:catAx>
    <c:valAx><c:axId val="222222222"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:delete val="0"/><c:axPos val="b"/><c:majorGridlines/><c:numFmt formatCode="General" sourceLinked="1"/><c:txPr><a:bodyPr/><a:lstStyle/><a:p><a:pPr><a:defRPr sz="1000"/></a:pPr><a:endParaRPr lang="es-MX"/></a:p></c:txPr><c:crossAx val="111111111"/></c:valAx>`
}
...
export function drawingXml(specs: ChartSpec[]): string {
  const anchors = specs
    .map((spec, i) => {
      ...
      const { cols, rows } = spec.size ?? DEFAULT_SIZE
